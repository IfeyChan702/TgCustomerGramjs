import { makeTgAccountPath } from './MyRedis.js';
const { createClient } = require('redis');
const myTgKey = "5b59828d-ee69-4c42-86e0-e2d69872e256";
// 远程 Redis 地址和密码（请换成你的真实配置）
const redisUrl = 'redis://:test123456@192.168.3.77:44513'; // 有密码加:yourpassword@，无密码去掉

const client = createClient({ url: redisUrl });

client.on('error', (err) => console.log('Redis Client Error', err));

(async () => {
  await client.connect();

  // 写入
  // await client.hSet('tg:account:85252021856', {
  //   apiId: '22059335',
  //   apiHash: '5eb343d4aa291469585740ecccba1852',
  //   session: ''
  // });
  // 读取
  (async () => {
    const path = await makeTgAccountPath('12345');
    console.log(path); // 输出: tg:register:12345
  })();

  const value = await client.hGetAll(makeTgAccountPath(myTgKey));
  console.log(value);

  await client.quit();
})();